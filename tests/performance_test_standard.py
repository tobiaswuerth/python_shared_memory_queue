...
max_tests_per_run = 10_000

out_dir = 'test_results'

def sender_process(size, ready_signal, done_signal, data_queue):
    ...
    out_file = os.path.join(out_dir, f'standard_{size:013d}_send.json')
...
def receiver_process(size, ready_signal, done_signal, data_queue):
    runs = min(max_tests_per_run, max_memory // size)
    ...
    out_file = os.path.join(out_dir, f'standard_{size:013d}_receive.json')
    with open(out_file, 'w') as f:
        json.dump(time_deltas, f)
...
if __name__ == "__main__":
    ...
    for size in package_sizes:
        ...
        signal_done = multiprocessing.Event()

        sender = multiprocessing.Process(target=sender_process, args=(size, signal_ready, signal_done, q_data))
        receiver = multiprocessing.Process(target=receiver_process, args=(size, signal_ready, signal_done, q_data))
